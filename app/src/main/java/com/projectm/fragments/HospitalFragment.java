...
import com.projectm.adapters.HospitalCardAdapter;
...
public class HospitalFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        Query query = FirebaseDatabase.getInstance().getReference().child("Hospitals").child("Specialization").child("Dental")
                .orderByChild("location").equalTo(s);

        FirebaseRecyclerOptions<HospitalCard> options =
                new FirebaseRecyclerOptions.Builder<HospitalCard>()
                        .setQuery(query, HospitalCard.class)
                        .build();
        //FirebaseDatabase.getInstance().getReference().child("Doctors").child("Specialization").child(s)
        adapter=new HospitalCardAdapter(options);
